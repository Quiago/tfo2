  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setSensorData((prevSensors) => {
        const lastSensor = prevSensors[prevSensors.length - 1];
        ...
        // Batch the dependent state updates
        ...
        if (newActions.length > 0) {
          setActionData((prevActions) => {
            // Remove actions whose timestamps fell out of the sliding window
            const windowStart =
              now - (pointCount - 1) * INTERVALS_MS[granularity];
            const filtered = prevActions.filter(
        ...
        return [...prevSensors.slice(1), newSensor];
      });
